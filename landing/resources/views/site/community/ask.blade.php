...
>
    <div class="hv-container max-w-2xl py-10">
        ...
        <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-50">{{ landing_t('community.ask_page_title') }}</h1>
        <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">{{ landing_t('community.ask_page_lead') }}</p>
        ...
        @if ($categories->isEmpty())
            <p class="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-100">
                {!! landing_t('community.ask_no_categories_html') !!}
            </p>
        @else
            <form method="post" action="{{ route('community.ask.store') }}" class="relative mt-8 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">{{ landing_t('community.label_category') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">{{ landing_t('community.label_title') }}</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">{{ landing_t('community.label_body') }}</label>
                    <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{{ landing_t('community.body_format_help') }}</p>
                    <x-admin.rich-editor
                        name="body"
                        :value="old('body')"
                        id="community-ask-body"
                        :placeholder="landing_t('community.ask_body_placeholder')"
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300">{{ landing_t('community.label_tags') }}</label>
                    <p class="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{{ landing_t('community.tags_help') }} <span class="font-mono">{{ landing_t('community.tags_help_example') }}</span>{{ landing_t('community.tags_help_close') }}</p>
                ...
                <details class="rounded-xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-900/40">
                    <summary class="cursor-pointer text-sm font-semibold text-slate-800 dark:text-slate-200">{{ landing_t('community.advanced_seo_toggle') }}</summary>
                    <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">{{ landing_t('community.advanced_seo_lead') }}</p>
                    <div class="mt-4 space-y-3">
                        <div>
                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300">{{ landing_t('community.meta_title_label') }}</label>
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300">{{ landing_t('community.meta_description_label') }}</label>
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="submit" class="rounded-xl bg-[rgb(var(--hv-brand-600)/1)] px-5 py-2.5 text-sm font-semibold text-white">{{ landing_t('community.publish') }}</button>
                    <a href="{{ route('community.index') }}" class="text-sm font-medium text-slate-600 underline hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100">{{ landing_t('community.cancel') }}</a>
                </div>
            </form>
        @endif
    </div>
