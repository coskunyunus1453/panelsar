        ...
        <div class="flex h-14 items-center justify-between gap-3 sm:h-[3.75rem]">
            ...
            <div class="flex shrink-0 items-center gap-2">
                ...
                <div class="hidden items-center gap-1.5 sm:flex">
                    @auth
                        <a href="{{ route('community.profile.edit') }}" class="shrink-0 rounded-full ring-1 ring-slate-200/90 hover:opacity-90 dark:ring-slate-600" title="Topluluk profili">
                        ...
                        <form method="post" action="{{ route('logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="rounded-lg border border-slate-300/80 px-2.5 py-1.5 text-xs font-semibold text-slate-700 dark:border-slate-600 dark:text-slate-200">Çıkış</button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" class="rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-200">Giriş</a>
                        <a href="{{ route('register') }}" class="rounded-lg bg-slate-900 px-2.5 py-1.5 text-xs font-semibold text-white dark:bg-slate-100 dark:text-slate-900">Kayıt</a>
                    @endauth
                </div>
