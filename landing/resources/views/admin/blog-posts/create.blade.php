<x-admin.layout title="Yeni blog yazısı">
    <form method="POST" action="{{ route('admin.blog-posts.store') }}" class="mx-auto max-w-2xl space-y-5">
        @csrf
        ...
        <div>
            ...
            @enderror
            <p class="mt-1 text-[11px] text-slate-500">Dil değiştirirseniz kategori listesi kayıttan sonra güncellenir; aynı dilde kategori seçin.</p>
        </div>
        ...
        <div>
            ...
            <textarea id="excerpt" name="excerpt" rows="3"
                      class="admin-field mt-1">{{ old('excerpt') }}</textarea>
            @error('excerpt')
                <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="admin-form-panel space-y-4 !shadow-none">
            ...
            <div>
                ...
                <input id="meta_title" name="meta_title" type="text" value="{{ old('meta_title') }}" class="admin-field mt-1" placeholder="Boşsa yazı başlığı kullanılır" />
                @error('meta_title')
                    <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <textarea id="meta_description" name="meta_description" rows="2" class="admin-field mt-1" placeholder="Boşsa özet veya içerikten türetilir">{{ old('meta_description') }}</textarea>
            ...
            <div>
                ...
                <input id="canonical_url" name="canonical_url" type="text" value="{{ old('canonical_url') }}" class="admin-field mt-1 font-mono text-sm" placeholder="/blog/yazi veya https://..." />
                @error('canonical_url')
                    <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            ...
            @error('published_at')
                <p class="mt-1 text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p>
            @enderror
            <p class="mt-1 text-[11px] text-slate-500">Boş bırakılabilir; yayında işaretlenince geçerli tarih kullanılmalıdır.</p>
        </div>
