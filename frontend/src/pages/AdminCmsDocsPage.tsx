...
const KIND = 'doc' as const

export default function AdminCmsDocsPage() {
    ...
    <div className="space-y-6">
      <div className="card overflow-hidden p-0">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-4 py-3 dark:border-gray-800">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('cms_admin.docs_heading')}</h2>
          <button type="button" className="btn-primary inline-flex items-center gap-2" onClick={startNew}>
            <Plus className="h-4 w-4" />
            {t('cms_admin.new_page')}
          </button>
        </div>
      ...
      {(creating || editing) && (
        <div className="card p-6">
          <h3 className="mb-4 text-base font-semibold text-gray-900 dark:text-white">
            {creating ? t('cms_admin.new_doc') : t('cms_admin.edit_doc')}
          </h3>
          <CmsMarkdownEditor
            locale={locale}
